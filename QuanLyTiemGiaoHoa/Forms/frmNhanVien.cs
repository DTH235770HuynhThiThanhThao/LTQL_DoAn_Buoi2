...
namespace QuanLyTiemGiaoHoa.Forms
{
    ...
    public partial class frmNhanVien : Form
    {
        ...
        {
            // Nếu đang tìm kiếm -> bấm lần nữa = reset
            if (dangTimKiem)
            {
                txtTimKiem.Clear();
                frmNhanVien_Load(sender, e);
            ...
            var ketQua = context.NhanVien
                .Where(x =>
                    x.HoVaTen.ToLower().Contains(tuKhoa) ||
        ...
        private void dataGridView_SelectionChanged(object sender, EventArgs e)
        {
            ...
            {
                bool quyen = Convert.ToBoolean(dataGridView.CurrentRow.Cells["QuyenHan"].Value);

                if (quyen == true)
                    cboQuyenHan.SelectedIndex = 0; // Admin
                else
                    cboQuyenHan.SelectedIndex = 1; // Nhân viên
            }
        }
    }
